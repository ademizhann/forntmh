  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      if (file.type.startsWith("text/") || file.name.endsWith(".json") || file.name.endsWith(".txt")) {
        ...
        const reader = new FileReader();
        reader.onload = (event) => {
          Tesseract.recognize(event.target.result, "eng+rus", { logger: (m) => console.log(m) })
            .then(({ data: { text } }) => setInput(text))
            .catch(() => alert("Ошибка OCR. Попробуйте другое изображение."));
        };
        reader.readAsDataURL(file);
      } else {
        alert("Поддерживаются только текстовые файлы, PDF, Word (.docx) и изображения");
      }
    } catch (err) {
        ...
        >
          ...
          >
            ...
            <input
              ...
              ref={fileInputRef}
              style={{ display: "none" }}
              accept="image/*,text/*,.json,.txt,.pdf,.docx"
